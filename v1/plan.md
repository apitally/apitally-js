...
- **Test scenarios:** (spool tests run against both disk and memory backends, py parity)
  ...
  - a batch whose records are individually valid but collectively over the file budget lands split across files, each closed file under the budget, payloads round-tripping complete (design-js §10 — JS-only, byte-bounded appends)
  - a single record whose serialized size exceeds the file budget is dropped with a warning; surrounding records still deliver (design-js §10 — JS-only)
...
- Express route templates ride registration-time capture (the mechanism dd-trace/New Relic/Elastic/OTel all use — router 2.x makes post-hoc readback impossible); the setup-ordering contract is documented loudly and violations degrade to cleared routes plus a warning. Express 4/5 structural differences (`app.handle`, router prototype shape) are covered by the version matrix.
