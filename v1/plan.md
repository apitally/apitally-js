...
- **Files:** `src/logPipeline.ts` (ported from py `shared/log_processor.py`), `src/logCapture.ts` (one module for all three patches — console and winston ported from v0 `loggers/{console,winston}.ts`, pino new via a write patch on the prototype that owns `pino.symbols.writeSym`, found by walking a probe instance's chain — spike 8; shared severity mapping and emit helpers live beside their callers), `tests/shared/logPipeline.test.ts`, `tests/shared/logCapture.test.ts` (one `describe` per library).
...
- D4's prototype patches (console, winston, pino) share D1's exposure class — peer internals: a peer major moving the write path changes what the patch sees, and another APM agent's wrapper may sit on the same prototypes. Mitigation: spike 8 pins the mechanism per supported major, the matrix runs the declared floors, and the patches are check-and-mark (design-js §4), so re-entry composes instead of stacking.
