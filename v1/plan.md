...
- **Approach:** per design-js §8: `useApitally` wraps `app.fetch` (span, fresh context, response observation — `onError`-synthesized responses return through it) and registers a thin inner middleware for context-bound data (`c.req.routePath`, consumer). The existing `onError` handler is wrapped through the runtime-accessible `errorHandler` property (duck-typed, defensively); 404-ness derives from route-match state plus response status. Request bodies are captured passively through Hono's body cache; response bodies via the ported `captureResponse` teeing helper (design-js §7). Integration drives the same canonical scenario set as U13 via `app.request()`, identical `it` strings and order.
