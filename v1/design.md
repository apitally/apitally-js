...
| Validation and server error capture | on |
...
- **Request stage, at SERVER span start**: `sample_on_request` resolves an effective rate — boolean maps to 1.0/0.0, a float in [0,1] is used as-is, absent/abstain falls back to `sample_rate`. A sampled-out request transmits no spans, request-scoped application logs, headers, or bodies, and skips their capture work; framework-specific buffers that necessarily fill before the span exists are discarded once the decision is known. Validation and server error capture remains independent per §8.
...
A request's trace and application-log detail is exported when the request completes — one behavior for every request. Ended descendants buffer in the span processor and the request's log records buffer in the log pipeline, keyed by the SERVER span id, until the response-stage decision. Buffers and in-flight map entries stay alive until that decision runs, so telemetry emitted during response streaming still buffers and flushes or discards with its request. Keep flushes descendants, then the SERVER span, then the request's logs — and from that point the request's late-ending spans and late log records export immediately without buffering. Drop discards all buffered trace and application-log detail, removes the request's span ids from the in-flight map so late telemetry falls to the lookup-miss rule and drops locally, and releases any stashed payloads unprocessed — a response-stage drop exports no trace detail and consumes no trace quota. Validation and server error capture remains independent. Buffers are bounded per request: 1,000 spans and 1,000 log records (Sentry parity); once a cap is reached, new arrivals are dropped and the earliest 1,000 are kept.
...
At transport completion, after the final status, parameterized route, and consumer are known, the request is committed exactly once:

- a validation group is added for each recognized validation detail;
- a server group is added only for a captured exception with final status 500;
- every other status adds no server group, and a deliberate 500 response without an exception adds no server group;
- `OPTIONS`, websockets, and requests without a parameterized route add nothing;
- trace sampling, response sampling, exclusion, and `capture_logs` do not participate in this decision.

The shared reporter normalizes and truncates before keying. It keeps lock-safe or runtime-equivalent maps for the aggregation identities in spec §9.2, with fixed limits of 100 validation groups and 100 server groups between drains. Existing groups continue incrementing at capacity; new identities are silently ignored. The latest non-empty Sentry event ID enriches a server group without changing its identity.

The reporter emits one root-context structured log event per group through the private logger provider. It atomically swaps the maps during each regular and final export cycle, emits outside the synchronization boundary, and drains immediately before the log processor is force-flushed. These records use the ordinary logs queue and spool behavior. They have no separate priority, retention policy, warning, or overflow counter.

There is no public validation capture API. Automatic framework recognition is the complete validation surface.

Cloud ingestion support for both event names MUST be deployed before an SDK version that emits them. Older cloud versions safely ignore the records, which would otherwise lose captured validation and server errors silently.

## 9. Logs

- **Capture surface**: each SDK captures application logs from its ecosystem's dominant logging interface(s) — the standard library logger where the language has one (Python `logging`, Go `slog`), the standard logging abstraction (.NET `ILogger`), or a chosen set of major logging libraries where no standard exists (JS: e.g. the contrib instrumentations for `winston`/`pino`, plus `console`). The chosen surface is recorded in the language design doc, favoring interfaces that can be captured without changes to the user's logging setup. Capture hooks in at the most permissive level (user-configured per-logger thresholds still apply) and routes records into Apitally's private logger provider — installed directly, never via an instrumentation entry point that wires the global provider. User-owned logger providers are never touched: logs emitted through the user's own OTel setup stay in the user's pipeline, like their spans stay in their exporters. Captured records carry code-location attributes (`code.file.path`, `code.line.number`, `code.function.name`) where the logging framework provides them — they feed the file/line display on application logs (spec §8). Default on; `capture_logs=false` disables only the application-log capture surface — the private logger provider and all SDK internal events are unaffected.
...
- **Drop rule**: application records that resolve no SERVER span id are dropped. Records emitted directly under the `apitally` instrumentation scope preserve the SDK internal events in spec §9, which have no request context.
...
- String log bodies and string attribute values are truncated to 2,048 characters at export (hard cut, no marker); non-string values pass through unmodified. `apitally`-scoped internal records are exempt and apply their event-specific limits from spec §9.

### Startup event

Emitted once per process at activation, per spec §9.1: a log record under scope `apitally` with event name `apitally.app.startup` and a JSON-string body carrying `framework`, `versions` (language runtime always included), `paths`, and `openapi` (omitted above 4,000,000 bytes; `paths` remain). Route/schema data is resolved lazily at emit time — routes commonly finalize after setup. Emitted directly on the private logger provider, bypassing the log capture surface. The `framework` value is informational — stored and displayed as the app's client (route normalization is keyed on the framework selected for the app in the dashboard, not on this value).
...
- **Final drain**: graceful shutdown (§4) runs one final cycle — drain pending validation and server error groups, flush the batch processors, collect metrics, close all current files, and send. Regular cycles also drain validation and server error groups immediately before forcing the log processor to flush. The final drain skips the inter-send pauses and ignores the per-cycle file cap: every pending file is sent, so a backlog built up during an outage is fully delivered instead of being stranded beyond the first ten files.
