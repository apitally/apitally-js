...
- **No existing tracer provider**: the SDK sets up its own tracer provider with an explicitly always-on sampler. A sampler is never Apitally's drop mechanism — the §5 processor is the only drop point — and coverage must not depend on upstream tracing-cost sampling: the OTel default sampler turns a SERVER span under an unsampled upstream `traceparent` into a non-recording span, violating spec §6.5. Passing the sampler explicitly also means the SDK never honors sampler-selection env vars. Accepted trade-off: such requests propagate sampled=1 downstream.
...
Idiomatic per language: entry-point verbs and option casing, callback/handler shapes, concurrency machinery, framework-adapter mechanics, dependency layout. Where this document says "thread", read "the language's background execution primitive".
