...
- Hono request bodies are read at transport completion through Hono's body cache (`c.req.arrayBuffer()`): served from the cache when the handler consumed the body through `HonoRequest` methods, read from the request otherwise — capture does not depend on the handler reading the body, matching the reference implementation's WSGI capture and v0 behavior. A body the handler consumed directly off `c.req.raw` cannot be re-read and is not captured.
