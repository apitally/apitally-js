...
- Hono request bodies are captured passively too, through Hono's body cache: at transport completion, only when the handler consumed the body (`c.req.raw.bodyUsed`), `c.req.arrayBuffer()` serves the already-buffered bytes via `HonoRequest`'s cache derivation — the SDK never reads the request stream itself. An unconsumed body is not captured (v0 read it after the response; dropped — a post-response network read can stall on an unfinished client and needs its own cap-aware stream reader), and a body consumed directly off `c.req.raw` bypasses the cache and is not captured. Both adapters share one contract: a body the app never reads is not captured.
...
- Mask callbacks: `maskRequestBody(body: Buffer, span)` returning `Buffer | null`; returning nothing, throwing, or returning the wrong type (a Promise included — mask callbacks are synchronous) yields `[REDACTED]`. Documented contract: may run later, off the request path, against an ended span snapshot.
...
- Optional peers (winston, pino — both CommonJS) are resolved with `node:module` `createRequire`, synchronously: activation is synchronous, so dynamic `import()` never sits on that path.
...
- Test conventions: two-tier layout (`tests/shared/<module>.test.ts` mirroring source modules; per-framework integration dirs with a uniform app fixture); subject-predicate naming (`describe` = module/adapter name, `it` = present-tense behavior predicate, no `should`), with shared scenarios using identical `it` strings and the same canonical order across framework files; files ordered core behavior → edge cases → failure paths → shutdown, hooks at top. Scenario selection is contract-derived — each test pins a spec MUST, a settled design decision, or a plausible regression (authority cited in the plan), and fails only when user-observable behavior regresses: documented gaps, internal mechanisms, and constants are never pinned — with the Python suite mined as evidence, not authority. No wall-clock sleeps or fake timers: deterministic seams only (callable worker cycle, force-flush reads, mtime manipulation). Assertions are exact-by-default: exact counts, full attribute equality, protobuf-decoded payloads in export tests; no snapshots. Coverage ownership: every behavior is asserted in exactly one home — the lowest layer that can observe it; `tests/shared/` owns core semantics, framework suites own adapter behavior, wiring, and the canonical cross-framework set (the only sanctioned duplication); helpers stay consolidated — extend an existing helper over adding a sibling.
...
3. otlp-transformer serialization byte-stability for identical input (dedup contract) and gzip determinism of the spool append path; also that `Buffer`/`Uint8Array` attribute values on hand-built export copies serialize to `bytesValue` at the pinned version (the TS `Attributes` type does not admit them — the export copy rides `toAnyValue`'s runtime handling).
