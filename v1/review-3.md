...
**Status:** Fixed. Inter-send pauses keep the event loop alive while a cycle is active; the recurring export timer remains unreferenced. Verified with a temporary child-process reproduction: pending files and a queued final drain complete before natural exit.
