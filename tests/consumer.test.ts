...
import { type ApitallyConsumer, consumerFromStringOrObject } from "../src/consumer.js";
...
describe("consumer", () => {
  ...
  it("accepts a numeric identifier and converts it to a string", () => {
    ...
    expect(consumerFromStringOrObject({ identifier: 123 as unknown as string })).toEqual({
      identifier: "123",
    });
  });

  it("drops empty name and group from a consumer object", () => {
    expect(consumerFromStringOrObject({ identifier: "acme", name: "  ", group: "" })).toEqual({
      identifier: "acme",
    });
  });
  ...
  it("returns no consumer for a missing, empty, or invalid identifier", () => {
    ...
    expect(consumerFromStringOrObject({ identifier: true as unknown as string })).toBeUndefined();
  });
});
